<template>
	<section>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" style="width: 100%;">
			<el-table-column prop="taskCode" label="采样单编号" width="" sortable>
			</el-table-column>
			<el-table-column prop="passengerName" label="被采样人姓名" width="" sortable>
			</el-table-column>
			<el-table-column prop="passengerCode" label="被采样人护照号" width="" sortable>
			</el-table-column>
			...
			<el-table-column prop="createTimeStr" label="创建时间" width="" sortable>
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog :title="$t('menu.bianji')" :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item :label="$t('menu.guanqumingcheng')" prop="customsCode">
					<el-cascader v-model="editForm.customsCode" :props="optionProps" :options="OrganizeListByCode" @change=""></el-cascader>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				<el-button @click.native="editFormVisible = false">{{$t('menu.quxiao')}}</el-button>
				<el-button type="primary" @click.native="editSubmit" :loading="editLoading">{{$t('menu.tijiao')}}</el-button>
			</div>
		</el-dialog>

		<!--新增界面-->
		<el-dialog :title="$t('menu.xinzeng')" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item :label="$t('menu.guanqumingcheng')" prop="customsCode">
					<el-cascader v-model="addForm.customsCode" :props="optionProps" :options="OrganizeListByCode" @change=""></el-cascader>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				<el-button @click.native="addFormVisible = false">{{$t('menu.quxiao')}}</el-button>
				<el-button type="primary" @click.native="addSubmit" :loading="addLoading">{{$t('menu.tijiao')}}</el-button>
			</div>
		</el-dialog>
	</section>
</template>
		...
		methods: {
			...
			getOfficerList() {
				let para = {
					...
					customsCode: this.customsCode_active?this.customsCode_active[this.customsCode_active.length-1]:'',
				...
				this.$refs.editForm.validate((valid) => {
					...
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.editForm.customsCode = this.editForm.customsCode[this.editForm.customsCode.length-1]
				...
				this.$refs.addForm.validate((valid) => {
					...
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.addForm.customsCode = this.addForm.customsCode[this.addForm.customsCode.length - 1]
